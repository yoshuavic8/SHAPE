...
export function analyzeSpiritualGifts(answers: Answers): CategoryScore[] {
...
export function analyzePersonality(answers: Answers): CategoryScore[] {
    ...
    Object.entries(scores)
        .filter(
            ([dimension, { count }]) => count > 0 && dimension !== "reflection"
        )
    ...
    const sensingScore =
        scores["sensing"].total / (scores["sensing"].count || 1);
    const intuitionScore =
        scores["intuition"].total / (scores["intuition"].count || 1);
    const thinkingScore =
        scores["thinking"].total / (scores["thinking"].count || 1);
    const feelingScore =
        scores["feeling"].total / (scores["feeling"].count || 1);
    const judgingScore =
        scores["judging"].total / (scores["judging"].count || 1);
    ...
    if (mbtiAnalysis) {
        ...
        results.push({
            category: `Deskripsi: ${mbtiAnalysis.title} - ${mbtiAnalysis.description.general}`,
            score: 5,
            percentage: 100,
        });
        ...
        // Add core values
        results.push({
            category: `Nilai Inti: ${mbtiAnalysis.description.coreValues}`,
            score: 5,
            percentage: 100,
        });
        ...
        mbtiAnalysis.strengths.dominantAbilities.forEach((ability, index) => {
            results.push({
                category: `Kemampuan #${index + 1}: ${ability.title}`,
        ...
        if (text && text.trim() !== "") {
            ...
            switch (questionId) {
                ...
                case "75":
                    ...
                    if (
                        lowerText.includes("menulis") ||
                        lowerText.includes("tulis")
                    ) {
                        scores["menulis"].total += 5;
                        scores["menulis"].count += 1;
                    }
    ...
    if (personalityType) {
        ...
        if (
            personalityType.includes("E") &&
            personalityType.includes("F") &&
            (scores["interpersonal"].count > 0 ||
                scores["public speaking"].count > 0)
        ) {
            results.push({
                category:
        ...
        if (text && text.trim() !== "") {
            ...
            switch (questionId) {
                ...
                case "84":
                    ...
                    insight =
                        "Menunjukkan ketahanan dan pertumbuhan pasca-trauma";
                    experienceThemes.resilience += 3;
                    break;
                case "85":
...
export interface ShapeRecommendations {
    ...
    bibleVerse: string;
    shapeProfile: Record<string, string>;
    shapeSynergy: string[];
...
export function generateRecommendations(
    ...
    if (personalityType) {
        const mbtiMinistryRecs =
            getMBTIMinistryRecommendations(personalityType);
    ...
    topAbility.forEach((ability) => {
        developmentRecommendations.push(
            `Kursus lanjutan dalam bidang ${ability}`
        );
    });
    ...
    const shapeProfile = {
        spiritual:
            topSpiritual.length > 0 ? topSpiritual[0] : "Tidak teridentifikasi",
        heart: topHeart.length > 0 ? topHeart[0] : "Tidak teridentifikasi",
        abilities:
            topAbility.length > 0 ? topAbility[0] : "Tidak teridentifikasi",
        personality: personalityType || "Tidak teridentifikasi",
        experiences:
            experienceThemes.length > 0
                ? experienceThemes[0]
    ...
    if (topHeart.length > 0 && topAbility.length > 0) {
        if (
            topHeart.includes("Anak-anak") &&
            topAbility.includes("Kreativitas")
        ) {
            shapeSynergy.push(
    ...
    if (
        ...
        learningPathways.push(
            "Pelatihan software produktivitas dan organisasi"
        );
    }
    ...
    if (personalityType) {
        ...
        if (personalityType.includes("E")) {
            learningPathways.push(
                "Pelatihan manajemen energi dan refleksi diri"
            );
        }
        ...
        if (personalityType.includes("S")) {
            learningPathways.push(
                "Pelatihan pemikiran inovatif dan kreativitas"
            );
        }
    }
...
export function analyzeResults(answers: Answers): AnalysisResults {
    const spiritualGifts = analyzeSpiritualGifts(answers);
    const heartDesire = analyzeHeartDesire(answers);
    const personality = analyzePersonality(answers); // Now includes abilities
    const experiences = analyzeExperiences(answers);
